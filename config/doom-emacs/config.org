...
First, fix an performance issue where the default ~C-u~ keybind calls =evil-scroll-up=,
which is stupidly slow.

#+begin_src emacs-lisp
(map! :n "C-u" #'evil-scroll-page-up
      :n "C-d" #'evil-scroll-page-down)
#+end_src
** Completion (=corfu=)
...
Use our password store as source of authentication information.
